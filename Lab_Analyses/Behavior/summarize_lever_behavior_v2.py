...
def summarize_imaged_lever_behavior(file, MIN_MOVE_NUM, MIN_T):
    ...
    for num, trial in enumerate(file.behavior_frames):
        ...
        d_trial = file.dispatcher_data.saved_history.ProtocolsSection_parsed_events[num]
        start_trial = np.round(bit_code[i_bitcode, 0] * 1000)  # time
        t0 = d_trial.states.bitcode[0]
        end_trial = int(
            start_trial + np.round((d_trial.states.state_0[1, 0] - t0) * 1000)
        )  # time

        if file.imaged_trials[num] == 1:
