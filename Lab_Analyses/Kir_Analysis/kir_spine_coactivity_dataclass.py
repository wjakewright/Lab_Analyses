...
class Kir_Coactivity_Data:
    ...
    near_vs_dist_activity_rate: np.ndarray
    ...
    near_vs_dist_volume: np.ndarray
    local_relative_vol: np.ndarray
    shuff_relative_vol: np.ndarray
    relative_vol_distribution: np.ndarray
    near_vs_dist_relative_volume: np.ndarray
    ...
    def save(self):
        ...
        fname = (
            f"{self.mouse_id}_{self.session}_{aname}_{mname}{pname}_kir_coactivity_data"
        )
        save_pickle(fname, self, save_path)


class Grouped_Kir_Coactivity_Data:
    """Class to group individual Kir_Coactivity_Data sets together"""
    ...
    def concatenate_data(self, data_list):
        ...
        together and store the attributes
        """
    ...
    def save(self):
        ...
        fname = f"{self.session}_{self.parameters['FOV type']}_{aname}_{mname}{pname}_grouped_kir_coactivity_data"
        save_pickle(fname, self, save_path)
