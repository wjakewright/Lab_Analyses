...
def spine_calcium_vs_volume(
    ...
    for mouse in mouse_list:
        datasets = load_spine_datasets(mouse, ["Early", "Middle"], fov_type=fov_type,)
        for FOV, dataset in datasets.items():
            ...
            pre_calcium = d_utils.subselect_data_by_idxs(pre_calcium, stable_idxs,)
            post_activity = d_utils.subselect_data_by_idxs(post_activity, stable_idxs,)
            ...
            for i in range(pre_activity.shape[1]):
                pre_stamps = t_stamps.get_activity_timestamps(pre_activity[:, i])
                post_stamps = t_stamps.get_activity_timestamps(post_activity[:, i])
            ...
            (pre_cal_traces, pre_calcium_amplitude, _,) = analyze_event_activity(
                pre_calcium,
                pre_onsets,
                activity_window=(-1, 2),
                center_onset=True,
            ...
            (post_cal_traces, post_calcium_amplitude, _,) = analyze_event_activity(
                post_calcium,
                post_onsets,
                activity_window=(-1, 2),
                center_onset=True,
        ...
        (enlarged, shrunken, stable,) = classify_plasticity(
            delta_volumes, (0.25, 0.5), norm=False
        )
        ...
        for key, value in plastic_groups.items():
            ...
            pre_means = [
                np.nanmean(x, axis=1) for x in pre_traces if type(x) == np.ndarray
            ]
            pre_means = np.vstack(pre_means)
            plastic_pre_trace_means[key] = np.nanmean(pre_means, axis=0)
            plastic_pre_trace_sems[key] = stats.sem(
                pre_means, axis=0, nan_policy="omit"
            )
            ...
            plastic_post_trace_sems[key] = stats.sem(
                post_means, axis=0, nan_policy="omit"
            )
