...
from Lab_Analyses.Spine_Analysis_v2.spine_utilities import (
    find_present_spines, load_spine_datasets)
from Lab_Analyses.Spine_Analysis_v2.structural_plasticity import (
    calculate_spine_dynamics, calculate_volume_change, classify_plasticity)
...
def plot_structural_plasticity(
    ...
    for mouse in apical_mice:
        temp_datasets = load_spine_datasets(mouse, ["Early", "Middle","Late"], fov_type="apical")
        for FOV, dataset in temp_datasets.items():
            ...
            for session in ["Early", "Middle", "Late"]:
                ...
                delta, _ = calculate_volume_change(
                    [temp_data.corrected_spine_volume, temp_data.corrected_followup_volume],
                    [temp_data.spine_flags, temp_data.followup_flags,],
                    exclude="Shaft Spine",
                    norm=False,
                )
                e, s, _ = classify_plasticity(
                    delta[-1], threshold=threshold, norm=False,
                )
                ...
                apical_enlarged_volumes[session].append(np.nanmean(np.array(delta[-1])[e]))
                apical_shrunken_volumes[session].append(np.nanmean(np.array(delta[-1])[s]))
            temp_density, temp_new, temp_elim = calculate_spine_dynamics(
                temp_flags, temp_positions, temp_groupings,
            )
    ...
    for mouse in basal_mice:
        temp_datasets = load_spine_datasets(mouse, ["Early", "Middle","Late"], fov_type="basal")
        for FOV, dataset in temp_datasets.items():
            temp_groupings = []
            temp_flags = []
            temp_positions = []
            for session in ["Early", "Middle", "Late"]:
                ...
                basal_enlarged_volumes[session].append(np.nanmean(np.array(delta[-1])[e]))
                basal_shrunken_volumes[session].append(np.nanmean(np.array(delta[-1])[s]))
            temp_density, temp_new, temp_elim = calculate_spine_dynamics(
                temp_flags, temp_positions, temp_groupings,
            )
            for j, session in enumerate(["Early", "Middle", "Late"]):
    ...
    plot_histogram(
        data=list((
            apical_volumes[0], basal_volumes[0]
        )),
        bins=hist_bins,
        stat="probability",
        avlines=None,
...
def plot_movement_related_activity(
    ...
    ## Event rates
    apical_event_rate = d_utils.subselect_data_by_idxs(apical_dataset.spine_activity_rate, apical_present)
    basal_event_rate = d_utils.subselect_data_by_idxs(basal_dataset.spine_activity_rate, basal_present)
    ## Movement identifiers
    apical_MRSs = d_utils.subselect_data_by_idxs(apical_dataset.movement_spines, apical_present)
    apical_nMRSs = d_utils.subselect_data_by_idxs(apical_dataset.nonmovement_spines, apical_present)
    basal_MRSs = d_utils.subselect_data_by_idxs(basal_dataset.movement_spines, basal_present)
    basal_nMRSs = d_utils.subselect_data_by_idxs(basal_dataset.nonmovement_spines, basal_present)
    ## Movement related activity
    apical_glu_traces = d_utils.subselect_data_by_idxs(apical_dataset.spine_movement_traces, apical_present)
    basal_glu_traces = d_utils.subselect_data_by_idxs(basal_dataset.spine_movement_traces, basal_present)
    apical_ca_traces = d_utils.subselect_data_by_idxs(apical_dataset.spine_movement_calcium_traces, apical_present)
    basal_ca_traces = d_utils.subselect_data_by_idxs(basal_dataset.spine_movement_calcium_traces, basal_present)
    apical_glu_amp = d_utils.subselect_data_by_idxs(apical_dataset.spine_movement_amplitude, apical_present)
    basal_glu_amp = d_utils.subselect_data_by_idxs(basal_dataset.spine_movement_amplitude, basal_present)
    apical_ca_amp = d_utils.subselect_data_by_idxs(apical_dataset.spine_movement_calcium_amplitude, apical_present)
    basal_ca_amp = d_utils.subselect_data_by_idxs(basal_dataset.spine_movement_calcium_amplitude, basal_present)
    apical_onset = d_utils.subselect_data_by_idxs(apical_dataset.spine_movement_onset, apical_present)
    basal_onset = d_utils.subselect_data_by_idxs(basal_dataset.spine_movement_onset, basal_present)

    # Get mean and sem traces for plotting
    ## Apical GluSnFR
    a_glu_traces = list(compress(apical_glu_traces, apical_MRSs))
    a_glu_means = [
        np.nanmean(x, axis=1) for x in a_glu_traces if type(x) == np.ndarray
    ]
    ...
    b_glu_means = [
        np.nanmean(x, axis=1) for x in b_glu_traces if type(x) == np.ndarray
    ]
    ...
    a_ca_means = [
        np.nanmean(x, axis=1) for x in a_ca_traces if type(x) == np.ndarray
    ]
    ...
    b_ca_means = [
        np.nanmean(x, axis=1) for x in b_ca_traces if type(x) == np.ndarray
    ]
    ...
    plot_pie_chart(
        data_dict={
            "MRSs": np.nansum(apical_MRSs),
            "nMRSs": np.nansum(apical_nMRSs)
        },
        title="Apical",
        figsize=(5,5),
    ...
    plot_pie_chart(
        data_dict={
            "MRSs": np.nansum(basal_MRSs),
            "nMRSs": np.nansum(basal_nMRSs)
        },
        title="Basal",
        figsize=(5,5),
    ...
    if test_type == "parametric":
        rate_t, rate_p = stats.ttest_ind(
            apical_event_rate, basal_ca_amp, nan_policy="omit",
        )
...
def plot_spine_movement_encoding(
    ...
    apical_reliability = d_utils.subselect_data_by_idxs(apical_reliability, apical_present)
    apical_specificity = d_utils.subselect_data_by_idxs(apical_specificity, apical_present)
    apical_fraction_rwd_mvmt = d_utils.subselect_data_by_idxs(apical_fraction_rwd_mvmt, apical_present)
    ...
    basal_fraction_rwd_mvmt = d_utils.subselect_data_by_idxs(basal_fraction_rwd_mvmt, basal_present)

    # Construct the figure
    fig, axes = plt.subplot_mosaic(
        """ABCDE""", figsize=figsize,
    )
    ...
    if test_type == "parametric":
        LMP_t, LMP_p = stats.ttest_ind(
            apical_LMP, basal_LMP, nan_policy="omit"
        )
        stereo_t, stereo_p = stats.ttest_ind(
            apical_sterotypy, basal_sterotypy, nan_policy="omit",
        )
        reli_t, reli_p = stats.ttest_ind(
            apical_reliability, basal_reliability, nan_policy="omit",
        )
        speci_t, speci_p, = stats.ttest_ind(
            apical_specificity, basal_specificity, nan_policy="omit",
        )
        ...
        LMP_t, LMP_p = stats.mannwhitneyu(
            apical_LMP[~np.isnan(apical_LMP)],
            basal_LMP[~np.isnan(basal_LMP)]
        )
...
def plot_local_coactivity(
    ...
    apical_distance_coactivity = d_utils.subselect_data_by_idxs(apical_distance_coactivity, apical_present)
    basal_distance_coactivity = d_utils.subselect_data_by_idxs(basal_distance_coactivity, basal_present)
    apical_distance_coactivity_norm = d_utils.subselect_data_by_idxs(apical_distance_coactivity_norm, apical_present)
    basal_distance_coactivity_norm = d_utils.subselect_data_by_idxs(basal_distance_coactivity_norm, basal_present)
    apical_local_coactivity = d_utils.subselect_data_by_idxs(apical_local_coactivity, apical_present)
    basal_local_coactivity = d_utils.subselect_data_by_idxs(basal_local_coactivity, basal_present)
    apical_shuff_coactivity = d_utils.subselect_data_by_idxs(apical_shuff_coactivity, apical_present)
    basal_shuff_coactivity = d_utils.subselect_data_by_idxs(basal_shuff_coactivity, basal_present)
    apical_local_coactivity_norm = d_utils.subselect_data_by_idxs(apical_local_coactivity_norm, apical_present)
    basal_local_coactivity_norm = d_utils.subselect_data_by_idxs(basal_local_coactivity_norm, basal_present)
    apical_shuff_coactivity_norm = d_utils.subselect_data_by_idxs(apical_shuff_coactivity_norm, apical_present)
    basal_shuff_coactivity_norm = d_utils.subselect_data_by_idxs(basal_shuff_coactivity_norm, basal_present)
    apical_near_vs_dist = d_utils.subselect_data_by_idxs(apical_near_vs_dist, apical_present)
    basal_near_vs_dist = d_utils.subselect_data_by_idxs(basal_near_vs_dist, basal_present)
    apical_near_vs_dist_norm = d_utils.subselect_data_by_idxs(apical_near_vs_dist_norm, apical_present)
    basal_near_vs_dist_norm = d_utils.subselect_data_by_idxs(basal_near_vs_dist_norm, basal_present)
    apical_fraction_coactive = d_utils.subselect_data_by_idxs(apical_fraction_coactive, apical_present)
    basal_fraction_coactive = d_utils.subselect_data_by_idxs(basal_fraction_coactive, basal_present)
    apical_fraction_participating = d_utils.subselect_data_by_idxs(apical_fraction_participating, apical_present)
    basal_fraction_participating = d_utils.subselect_data_by_idxs(basal_fraction_participating, basal_present)
    apical_coactive_num = d_utils.subselect_data_by_idxs(apical_coactive_num, apical_present)
    basal_coactive_num = d_utils.subselect_data_by_idxs(basal_coactive_num, basal_present)
    ...
    fig, axes = plt.subplot_mosaic(
        ...
        .K..LM
        """,
        figsize=figsize, width_ratios=[2,1,2,2,1,1]
    )
    ...
    ## histogram
    plot_cummulative_distribution(
        data=list((apical_local_coactivity, apical_shuff_coactivity,)),
        plot_ind=True,
        title="Apical",
    ...
    ## histogram
    plot_cummulative_distribution(
        data=list((basal_local_coactivity, basal_shuff_coactivity,)),
        plot_ind=True,
        title="Basal",
        xtitle=f"Local coactivity (event/min)",
        xlim=(0, None),
    ...
    ## histogram
    plot_cummulative_distribution(
        data=list((apical_local_coactivity_norm, apical_shuff_coactivity_norm,)),
        plot_ind=True,
        title="Apical",
    ...
    ## histogram
    plot_cummulative_distribution(
        data=list((basal_local_coactivity_norm, basal_shuff_coactivity_norm,)),
        plot_ind=True,
        title="Basal",
        xtitle=f"Local norm. coactivity",
        xlim=(0, None),
    ...
    if test_type == "parametric":
        ...
        frac_coactive_t, frac_coactive_p = stats.ttest_ind(
            apical_fraction_coactive, basal_fraction_participating, nan_policy="omit",
        )
        frac_part_t, frac_part_p = stats.ttest_ind(
            apical_fraction_participating, basal_fraction_participating, nan_policy="omit",
        )
    ...
    apical_above_norm, apical_below_norm = t_utils.test_against_chance(
        apical_local_coactivity_norm, apical_shuff_coactivity_norm,
    )
    basal_above, basal_below = t_utils.test_against_chance(
        basal_local_coactivity, basal_shuff_coactivity,
    )
    basal_above_norm, basal_below_norm = t_utils.test_against_chance(
        basal_local_coactivity_norm, basal_shuff_coactivity_norm,
    )

    # Organize results
    results_dict = {
        "Comparison": ["Local CoA", "Local Norm. CoA", "Near vs Dist", "Near vs Dist Norm", "Frac. CoA", "Frac Part.", "Coactive Num."],
        "stat": [coactivity_t, coactivity_norm_t, near_dist_t, near_dist_norm_t, frac_coactive_t, frac_part_t, num_t],
        "p-val": [coactivity_p, coactivity_norm_p, near_dist_p, near_dist_norm_p, frac_coactive_p, frac_part_p, num_p]
    }
    results_df = pd.DataFrame.from_dict(results_dict)
    results_df.update(results_df[["p-val"]].applymap("{:.4E}".format))
...
def plot_dendrite_coactivity(
    ...
    apical_coactivity_rate = d_utils.subselect_data_by_idxs(apical_dataset.all_dendrite_coactivity_rate, apical_present)
    basal_coactivity_rate = d_utils.subselect_data_by_idxs(basal_dataset.all_dendrite_coactivity_rate, basal_present)
    apical_conj_coactivity_rate = d_utils.subselect_data_by_idxs(apical_dataset.conj_dendrite_coactivity_rate, apical_present)
    basal_conj_coactivity_rate = d_utils.subselect_data_by_idxs(basal_dataset.conj_dendrite_coactivity_rate, basal_present)
    apical_nonconj_coactivity_rate = d_utils.subselect_data_by_idxs(apical_dataset.nonconj_dendrite_coactivity_rate, apical_present)
    basal_nonconj_coactivity_rate = d_utils.subselect_data_by_idxs(basal_dataset.nonconj_dendrite_coactivity_rate, basal_present)
    apical_frac_conj = d_utils.subselect_data_by_idxs(apical_dataset.fraction_conj_events, apical_present)
    basal_frac_conj = d_utils.subselect_data_by_idxs(basal_dataset.fraction_conj_events, basal_present)
    ## Event properties
    sampling_rate = apical_dataset.parameters["Sampling Rate"]
    activity_window = apical_dataset.parameters["Activity Window"]
    if apical_dataset.parameters["zscore"]:
    ...
    apical_glu_traces = d_utils.subselect_data_by_idxs(apical_dataset.all_spine_coactive_traces, apical_present)
    basal_glu_traces = d_utils.subselect_data_by_idxs(basal_dataset.all_spine_coactive_traces, basal_present)
    apical_ca_traces = d_utils.subselect_data_by_idxs(apical_dataset.all_spine_coactive_calcium_traces, apical_present)
    basal_ca_traces = d_utils.subselect_data_by_idxs(basal_dataset.all_spine_coactive_calcium_traces, basal_present)
    apical_dend_traces = d_utils.subselect_data_by_idxs(apical_dataset.all_dendrite_coactive_traces, apical_present)
    basal_dend_traces = d_utils.subselect_data_by_idxs(basal_dataset.all_dendrite_coactive_traces, basal_present)
    apical_glu_amp = d_utils.subselect_data_by_idxs(apical_dataset.all_spine_coactive_amplitude, apical_present)
    basal_glu_amp = d_utils.subselect_data_by_idxs(basal_dataset.all_spine_coactive_amplitude, basal_present)
    apical_ca_amp = d_utils.subselect_data_by_idxs(apical_dataset.all_spine_coactive_calcium_amplitude, apical_present)
    basal_ca_amp = d_utils.subselect_data_by_idxs(basal_dataset.all_spine_coactive_calcium_amplitude, basal_present)
    apical_dend_amp = d_utils.subselect_data_by_idxs(apical_dataset.all_dendrite_coactive_amplitude, apical_present)
