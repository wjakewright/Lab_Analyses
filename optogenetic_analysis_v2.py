...
class optogenetic_analysis():
    ...
    def __init__(self,grouped,data,method,sampling_rate=30,window=[-2,2],
                 vis_window=None,stim_len=1,zscore=False,spines=False):
    ...
    def group_data_analysis(self,method):
        ...
        for i, (dFoF, itis) in enumerate(zip(self.dFoF,self.itis)):
            ...
            roi_stims, roi_means = util.get_trace_mean_sem(activity=dFoF,
    ...
    def single_data_analysis(self,method):
        ...
        itis = self.itis
        ROIs = self.ROIs
        all_befores,all_afters = util.get_before_after_means(activity=dFoF,
                                                            timestamps=itis,
                                                            window=self.window,
                                                            sampling_rate=self.sampling_rate,
                                                            offset=False,
                                                            single=False)
        new_timestamps = [i[0] for i in itis]
        roi_stim_epochs, roi_mean_sems = util.get_trace_mean_sem(activity=dFoF,
                                                                timestamps=new_timestamps,
                                                                window=self.new_window,
                                                                sampling_rate=self.sampling_rate)
        results_dict, results_df, _ = util.significance_testing(imaging=dFoF,
                                                                timestamps=itis,
                                                                window=self.window,
                                                                sampling_rate=self.sampling_rate,
                                                                method=method)

        return [ROIs,all_befores,all_afters,roi_stim_epochs,
...
class power_curve():
    ...
    def __init__(self,grouped,data,powers,method,sampling_rate=30,window=[2,2],vis_window=None,
        ...
        self.grouped = grouped
        if grouped is True:
            data = [list(i) for i in data] # convert the list of tuples to list of lists
        self.data = data
        self.powers = powers
        self.method = method
        self.sampling_rate = sampling_rate
        ...
        opto_objs = []
        for dataset in data:
            opto_obj = optogenetic_analysis(self.grouped,dataset,self.method,self.sampling_rate,
                                            self.window,self.vis_window,self.stim_len,self.zscore,
                                            self.spines)
